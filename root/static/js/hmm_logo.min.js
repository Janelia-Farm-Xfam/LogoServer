...
function(){d(this.dom_element).parent().before('<div class="logo_xaxis" class="centered" style="margin-left:40px"><p class="xaxis_text" style="width:10em;margin:1em auto">Model Position</p></div>')};this.render_y_axis_label=function(){d(this.dom_element).parent().before('<canvas class="logo_yaxis" height="300" width="40"></canvas>');var c=d(this.called_on).find(".logo_yaxis");Math.abs(this.data.max_height);isNaN(this.data.min_height_obs)||parseInt(this.data.min_height_obs,10);k()||(c[0]=G_vmlCanvasManager.initElement(c[0]));
c=c[0].getContext("2d");c.beginPath();c.moveTo(40,1);c.lineTo(30,1);c.moveTo(40,256);c.lineTo(30,256);c.moveTo(40,128);c.lineTo(30,128);c.lineWidth=1;c.strokeStyle="#666666";c.stroke();c.fillStyle="#666666";c.textAlign="right";c.font="bold 10px Arial";c.textBaseline="top";c.fillText(this.data.max_height.toFixed(1),28,0);c.textBaseline="middle";c.fillText(parseFloat(this.data.max_height/2).toFixed(1),28,128);c.fillText("0",28,256);c.save();c.translate(5,this.height/2);c.rotate(-Math.PI/2);c.textAlign=
"center";c.font="normal 12px Arial";c.fillText("Information Content",1,0);c.restore()};this.render_x_axis_label();this.render_y_axis_label();this.render_with_text=function(c,a,d,q){var g=0,p=c,h=0,h=Math.abs(this.data.max_height),l=isNaN(this.data.min_height_obs)?0:parseInt(this.data.min_height_obs,10),h=h+Math.abs(l),h=Math.round(100*Math.abs(this.data.max_height)/h);Math.round(256*h/100);a+3<=this.end&&(a+=3);for(h=c;h<=a;h++){if(this.data.mmline&&1===this.data.mmline[h-1])this.contexts[d].fillStyle=
"#cccccc",this.contexts[d].fillRect(g,10,this.zoomed_column,this.height-40);else if(c=this.data.height_arr[h-1],l=[],c){for(var r=0,s=c.length,n=0,n=0;n<s;n++){var w=c[n].split(":",2),m=g+this.zoomed_column/2,v=null;if(0.01<w[1]){var v=parseFloat(w[1])/this.data.max_height,w=255-r,t=255*v;k()||(w+=t*(v/2));l[n]=[t,this.zoomed_column,m,w];r+=t}}for(n=s;0<=n;n--)l[n]&&this.letters[c[n][0]].draw(this.contexts[d],l[n][0],l[n][1],l[n][2],l[n][3])}f(this.contexts[d],g,this.height-15,5);f(this.contexts[d],
g,this.height-30,5);f(this.contexts[d],g,this.height-45,5);0.7>this.zoom?0===h%5&&this.draw_column_divider({context_num:d,x:g,fontsize:10,column_num:p,ralign:!0}):this.draw_column_divider({context_num:d,x:g,fontsize:q,column_num:p});c=this.data.delete_probs[h-1]/100;l=this.height-35;r="#ffffff";s="#555555";0.25<c?(r="#2171b5",s="#ffffff"):0.15<c?r="#6baed6":0.05<c&&(r="#bdd7e7");e(this.contexts[d],g,l,c,q,this.zoomed_column,r,s);c=this.contexts[d];l=g;r=this.height;s=this.zoomed_column;n=this.data.insert_probs[h-
1]/100;m=r-20;v="#ffffff";w="#555555";0.1<n?(v="#d7301f",w="#ffffff"):0.05<n?v="#fc8d59":0.03<n&&(v="#fdcc8a");e(c,l,m,n,q,s,v,w);0.03<n&&f(c,l+s,r-30,-30-r,v);c=this.data.insert_lengths[h-1];l="#ffffff";r="#555555";9<c?(l="#d7301f",r="#ffffff"):7<c?l="#fc8d59":4<c&&(l="#fdcc8a");e(this.contexts[d],g,this.height-5,c,q,this.zoomed_column,l,r);g+=this.zoomed_column;p++}b(this.contexts[d],this.height-15,this.total_width);b(this.contexts[d],this.height-30,this.total_width);b(this.contexts[d],this.height-
...
0,p=c,h=0,h=Math.abs(this.data.max_height),l=Math.abs(this.data.min_height_obs),h=h+l,h=Math.round(100*Math.abs(this.data.max_height)/h);Math.round(256*h/100);l=10;for(h=c;h<=a;h++){if(this.data.mmline&&1===this.data.mmline[h-1])this.contexts[e].fillStyle="#cccccc",this.contexts[e].fillRect(g,10,this.zoomed_column,this.height-40);else{c=this.data.height_arr[h-1];for(var r=0,s=c.length,n=0,n=0;n<s;n++){var k=c[n].split(":",2);if(0.01<k[1]){var m=parseFloat(k[1])/this.data.max_height,v=g,m=256*m,t=
256-r-m;d?(this.contexts[e].strokeStyle=this.colors[k[0]],this.contexts[e].strokeRect(v,t,this.zoomed_column,m)):(this.contexts[e].fillStyle=this.colors[k[0]],this.contexts[e].fillRect(v,t,this.zoomed_column,m));r+=m}}}0.2>this.zoom?l=20:0.3>this.zoom&&(l=10);0===h%l&&(f(this.contexts[e],g+this.zoomed_column,this.height-30,parseFloat(this.height),"#dddddd"),f(this.contexts[e],g+this.zoomed_column,0,5),c=this.contexts[e],r=g-2,s=this.zoomed_column,n=p,c.font="10px Arial",c.textAlign="right",c.fillStyle=
"#666666",c.fillText(n,r+s/2,10));c=this.contexts[e];r=g;s=this.height-42;n=this.zoomed_column;k=this.data.insert_probs[h-1]/100;v=this.data.insert_lengths[h-1];m=this.data.delete_probs[h-1]/100;t="#ffffff";0.1<k?t="#d7301f":0.05<k?t="#fc8d59":0.03<k&&(t="#fdcc8a");c.fillStyle=t;c.fillRect(r,s+12,n,10);t="#ffffff";9<v?t="#d7301f":7<v?t="#fc8d59":4<v&&(t="#fdcc8a");c.fillStyle=t;c.fillRect(r,s+24,n,10);t="#ffffff";0.25<m?t="#2171b5":0.15<m?t="#6baed6":0.05<m&&(t="#bdd7e7");c.fillStyle=t;c.fillRect(r,
...
c=b.data.probs_arr[e-1].slice(0).reverse();p=Math.ceil(c.length/5);for(h=0;h<p;h++)g=1<p&&h<p-1?g+('<th>Residue</th><th class="odd">'+l+"</th>"):g+("<th>Residue</th><th>"+l+"</th>");f.append(d(g+"</tr>"));for(h=0;5>h;h++){k+="<tr>";for(l=h;c[l];)g=c[l].split(":",2),k=1<p&&15>l?k+("<td>"+g[0]+'</td><td class="odd">'+g[1]+"</td>"):k+("<td>"+g[0]+"</td><td>"+g[1]+"</td>"),l+=5;k+="</tr>"}f.append(d(k));d(a.column_info).empty().append(d("<p> Column:"+e+"</p>")).append(f).show()});d(document).bind(this.attr("id")+
".scrolledTo",function(a,e,f,d){b.render({target:e})});d(document).keydown(function(a){a.ctrlKey||(61!==a.which&&107!==a.which||b.change_zoom({distance:0.1,direction:"+"}),109!==a.which&&0!==a.which||b.change_zoom({distance:0.1,direction:"-"}))})}else d("#logo").replaceWith(d("#no_canvas").html());return b}})(jQuery);/*
...
function(b,c){var d=u++;a[d]=b;null===f&&(f=setTimeout(function(){var b=k(),c=a,e=g(c);a={};f=null;for(var d=0,u=e.length;d<u;d++)c[e[d]](b)},1E3/60));return d};d.cancelRequestAnimationFrame=function(b){delete a[b];m(a)&&(clearTimeout(f),f=null)}}}})(this);
