...
5E3;this.scrollme=null;this.previous_target=0;this.rendered=[];this.previous_zoom=0;this.render=function(a){if(this.data){var a=a||{},b=a.zoom||this.zoom,f=a.target||1,k=c(this.dom_element).parent().width(),d=1,g=0;if(f!==this.previous_target){this.previous_target=f;a.start&&(this.start=a.start);a.end&&(this.end=a.end);0.1>=b?b=0.1:1<=b&&(b=1);this.zoom=b;var a=this.end||this.data.height_arr.length,i=this.start||1,a=a>this.data.height_arr.length?this.data.height_arr.length:a,a=a<i?i:a,i=i>a?a:i,i=
1<i?i:1;this.y=this.height-20;this.max_width=this.column_width*(a-i+1);k>this.max_width&&(b=1,this.zoom_enabled=!1);this.zoom=b;this.zoomed_column=this.column_width*b;this.total_width=this.zoomed_column*(a-i+1);if(1>b)for(;this.total_width<k&&!(this.zoom+=0.1,this.zoomed_column=this.column_width*this.zoom,this.total_width=this.zoomed_column*(a-i+1),this.zoom_enabled=!1,1<=b););f>this.total_width&&(f=this.total_width);c(this.dom_element).attr({width:this.total_width+"px"}).css({width:this.total_width+
"px"});k=Math.ceil(this.total_width/this.canvas_width);this.columns_per_canvas=Math.ceil(this.canvas_width/this.zoomed_column);this.previous_zoom!==this.zoom&&(c(this.dom_element).find("canvas").remove(),this.previous_zoom=this.zoom,this.rendered=[]);this.canvases=[];this.contexts=[];for(g=0;g<k;g++){var e=this.columns_per_canvas*g+i,m=e+this.columns_per_canvas-1;m>a&&(m=a);var n=(m-e+1)*this.zoomed_column;n>d&&(d=n);var r=d*g,t=r+n;if(f<t+t/2&&f>r-r/2&&1!==this.rendered[g]){var r=this.canvases,h=
g,p=this.dom_element,u=this.height,x=n,z=g,A=d,v=c(p).find("#canv_"+z);v.length||(c(p).append('<canvas class="canvas_logo" id="canv_'+z+'"  height="'+u+'" width="'+x+'" style="left:'+A*z+'px"></canvas>'),v=c(p).find("#canv_"+z));c(v).attr("width",x).attr("height",u);l()||(v[0]=G_vmlCanvasManager.initElement(v[0]));r[h]=v[0];this.contexts[g]=this.canvases[g].getContext("2d");this.contexts[g].setTransform(1,0,0,1,0,0);this.contexts[g].clearRect(0,0,n,this.height);this.contexts[g].fillStyle="#ffffff";
this.contexts[g].fillRect(0,0,t,this.height);12<this.zoomed_column?(n=parseInt(10*b,10),n=10<n?10:n,this.render_with_text(e,m,g,n)):this.render_with_rects(e,m,g);this.rendered[g]=1}}!this.scrollme&&l()&&(this.scrollme=new EasyScroller(c(this.dom_element)[0],{scrollingX:1,scrollingY:0,eventTarget:this.called_on}));1!==f&&l()&&this.scrollme.reflow()}}};this.render_x_axis_label=function(){c(this.dom_element).parent().before('<div class="logo_xaxis" class="centered" style="margin-left:40px"><p class="xaxis_text" style="width:10em;margin:1em auto">Model Position</p></div>')};
...
a.textBaseline="top";a.fillText(this.data.max_height.toFixed(1),28,0);a.textBaseline="middle";a.fillText(parseFloat(this.data.max_height/2).toFixed(1),28,128);a.fillText("0",28,256);a.save();a.translate(5,this.height/2);a.rotate(-Math.PI/2);a.textAlign="center";a.font="normal 12px Arial";a.fillText("Information Content",1,0);a.restore()};this.render_x_axis_label();this.render_y_axis_label();this.render_with_text=function(a,b,c,e){var j=0,g=a,i=0,i=Math.abs(this.data.max_height)+Math.abs(this.data.min_height_obs),
i=Math.round(100*Math.abs(this.data.max_height)/i);Math.round(256*i/100);b+3<=this.end&&(b+=3);for(i=a;i<=b;i++){if(this.data.mmline&&1===this.data.mmline[i-1])this.contexts[c].fillStyle="#cccccc",this.contexts[c].fillRect(j,10,this.zoomed_column,this.height-40);else if(a=this.data.height_arr[i-1])for(var q=0,m=a.length,n=0,n=0;n<m;n++){var r=a[n].split(":",2),t=j+this.zoomed_column/2,h=(0.0024*this.zoomed_column+0.0405).toFixed(2),p=null;if(0.01<r[1]){var p=parseFloat(r[1])/this.data.max_height,
u=254-q,x=258*p;l()||(u+=x*(p/2));this.contexts[c].font="bold 350px Arial";this.contexts[c].textAlign="center";this.contexts[c].fillStyle=this.colors[r[0]];this.contexts[c].transform(h,0,0,p,t,u);this.contexts[c].fillText(r[0],0,0);this.contexts[c].setTransform(1,0,0,1,0,0);q+=x}}k(this.contexts[c],j,this.height-15,5);k(this.contexts[c],j,this.height-30,5);k(this.contexts[c],j,this.height-45,5);0.7>this.zoom?0===i%5&&this.draw_column_divider({context_num:c,x:j,fontsize:10,column_num:g,ralign:!0}):
this.draw_column_divider({context_num:c,x:j,fontsize:e,column_num:g});a=this.data.delete_probs[i-1]/100;q=this.height-35;m="#ffffff";n="#555555";0.25<a?(m="#2171b5",n="#ffffff"):0.15<a?m="#6baed6":0.05<a&&(m="#bdd7e7");d(this.contexts[c],j,q,a,e,this.zoomed_column,m,n);a=this.contexts[c];q=j;m=this.height;n=this.zoomed_column;r=this.data.insert_probs[i-1]/100;t=m-20;h="#ffffff";p="#555555";0.1<r?(h="#d7301f",p="#ffffff"):0.05<r?h="#fc8d59":0.03<r&&(h="#fdcc8a");d(a,q,t,r,e,n,h,p);0.1<r&&k(a,q+n,m-
30,-30-m,h);a=this.data.insert_lengths[i-1];q="#ffffff";m="#555555";9<a?(q="#d7301f",m="#ffffff"):7<a?q="#fc8d59":4<a&&(q="#fdcc8a");d(this.contexts[c],j,this.height-5,a,e,this.zoomed_column,q,m);j+=this.zoomed_column;g++}f(this.contexts[c],this.height-15,this.total_width);f(this.contexts[c],this.height-30,this.total_width);f(this.contexts[c],this.height-45,this.total_width);f(this.contexts[c],0,this.total_width)};this.draw_column_divider=function(a){var b=a.ralign?a.x+this.zoomed_column:a.x,f=a.ralign?
a.x+2:a.x;k(this.contexts[a.context_num],b,this.height-30,-30-this.height,"#dddddd");k(this.contexts[a.context_num],b,0,5);var b=this.contexts[a.context_num],c=this.zoomed_column,d=a.column_num,g=a.ralign;b.font=a.fontsize+"px Arial";b.textAlign=g?"right":"center";b.fillStyle="#666666";b.fillText(d,f+c/2,10)};this.render_with_rects=function(a,b,c){var d=0,j=a,g=0,g=Math.abs(this.data.max_height),i=Math.abs(this.data.min_height_obs),g=Math.round(100*Math.abs(this.data.max_height)/(g+i));Math.round(256*
g/100);i=10;for(g=a;g<=b;g++){if(this.data.mmline&&1===this.data.mmline[g-1])this.contexts[c].fillStyle="#cccccc",this.contexts[c].fillRect(d,10,this.zoomed_column,this.height-40);else for(var a=this.data.height_arr[g-1],e=0,m=a.length,n=0,n=0;n<m;n++){var r=a[n].split(":",2);if(0.01<r[1]){var l=parseFloat(r[1])/this.data.max_height,h=d,l=256*l,p=256-e-l;this.contexts[c].fillStyle=this.colors[r[0]];this.contexts[c].fillRect(h,p,this.zoomed_column,l);e+=l}}0.2>this.zoom?i=20:0.3>this.zoom&&(i=10);
0===g%i&&(k(this.contexts[c],d+this.zoomed_column,this.height-30,parseFloat(this.height),"#dddddd"),k(this.contexts[c],d+this.zoomed_column,0,5),a=this.contexts[c],e=d-2,m=this.zoomed_column,n=j,a.font="10px Arial",a.textAlign="right",a.fillStyle="#666666",a.fillText(n,e+m/2,10));a=this.contexts[c];e=d;m=this.height-42;n=this.zoomed_column;r=this.data.insert_probs[g-1]/100;h=this.data.insert_lengths[g-1];l=this.data.delete_probs[g-1]/100;p="#ffffff";0.1<r?p="#d7301f":0.05<r?p="#fc8d59":0.03<r&&(p=
...
1>b.zoom?b.change_zoom({target:1,offset:a,column:d}):b.change_zoom({target:0.3,offset:a,column:d})});b.column_info&&k.bind("click",function(a){var d=f,k=c('<table class="logo_col_info"></table>'),e="<tr>",j="",g=c(this).offset(),a=parseInt(a.pageX-g.left,10);c(this).parent().offset();for(var d=d.columnFromCoordinates(a),d=f.data.height_arr[d-1].slice(0).reverse(),i=Math.ceil(d.length/5),a=g=a=0;a<i;a++)e+="<th>Residue</th><th>Score</th>";k.append(c(e+"</tr>"));for(a=0;5>a;a++){j+="<tr>";for(g=a;d[g];)e=
d[g].split(":",2),j+="<td>"+e[0]+"</td><td>"+e[1]+"</td>",g+=5;j+="</tr>"}k.append(c(j));c(b.column_info).empty().append(k)});c(document).bind(this.attr("id")+".scrolledTo",function(a,b){f.render({target:b})});c(document).keydown(function(a){a.ctrlKey||((61===a.which||107===a.which)&&f.change_zoom({distance:0.1,direction:"+"}),(109===a.which||0===a.which)&&f.change_zoom({distance:0.1,direction:"-"}))})}else c("#logo").replaceWith(c("#no_canvas").html());return f}})(jQuery);/*
...
(function(c){var l=Date.now||function(){return+new Date},e={},h=1;c.core?core.effect||(core.effect={}):c.core={effect:{}};core.effect.Animate={stop:function(b){var f=null!=e[b];f&&(e[b]=null);return f},isRunning:function(b){return null!=e[b]},start:function(b,f,c,d,a,y){var s=l(),w=s,j=0,g=0,i=h++;y||(y=document.body);if(0===i%20){var q={},m;for(m in e)q[m]=!0;e=q}var n=function(m){var m=!0!==m,h=l();if(!e[i]||f&&!f(i))e[i]=null,c&&c(60-g/((h-s)/1E3),i,!1);else{if(m)for(var q=Math.round((h-w)/(1E3/
60))-1,p=0;p<Math.min(q,4);p++)n(!0),g++;d&&(j=(h-s)/d,1<j&&(j=1));q=a?a(j):j;(!1===b(q,h,m)||1===j)&&m?(e[i]=null,c&&c(60-g/((h-s)/1E3),i,1===j||null==d)):m&&(w=h,requestAnimationFrame(n,y))}};e[i]=!0;requestAnimationFrame(n,y);return i}}})(this);
...
j=this.__scrollTop,g=this.__zoomLevel;if(null!=k&&this.options.zooming){var i=g,g=g/this.__lastScale*k,g=Math.max(Math.min(g,this.options.maxZoom),this.options.minZoom);if(i!==g){var q=d-this.__clientLeft,m=b-this.__clientTop,h=(q+h)*g/i-q,j=(m+j)*g/i-m;this.__computeScrollMax(g)}}if(this.__enableScrollX&&(h-=e,i=this.__maxScrollLeft,h>i||0>h))h=this.options.bouncing?h+e/2:h>i?i:0;if(this.__enableScrollY&&(j-=l,e=this.__maxScrollTop,j>e||0>j))this.options.bouncing?(j+=l/2,!this.__enableScrollX&&null!=
this.__refreshHeight&&(!this.__refreshActive&&j<=-this.__refreshHeight?(this.__refreshActive=!0,this.__refreshActivate&&this.__refreshActivate()):this.__refreshActive&&j>-this.__refreshHeight&&(this.__refreshActive=!1,this.__refreshDeactivate&&this.__refreshDeactivate()))):j=j>e?e:0;60<a.length&&a.splice(0,30);a.push(h,j,c);this.__publish(h,j,g)}else l=this.options.locking?3:0,h=Math.abs(d-this.__initialTouchLeft),j=Math.abs(b-this.__initialTouchTop),this.__enableScrollX=this.options.scrollingX&&
...
f;a.__scheduledZoom=e;var s=a.__scrollLeft,w=a.__scrollTop,j=a.__zoomLevel,g=b-s,i=f-w,q=e-j;a.__isAnimating=core.effect.Animate.start(function(b,c,d){d&&(a.__scrollLeft=s+g*b,a.__scrollTop=w+i*b,a.__zoomLevel=j+q*b,a.__callback&&a.__callback(a.__scrollLeft,a.__scrollTop,a.__zoomLevel))},function(b){return a.__isAnimating===b},function(b,c){c===a.__isAnimating&&(a.__isAnimating=!1);a.options.zooming&&a.__computeScrollMax()},250,h?c:l)}else a.__scheduledLeft=a.__scrollLeft=b,a.__scheduledTop=a.__scrollTop=
