...
this.canvas_width=5E3;this.letters={};a=null;for(a in this.colors)this.letters[a]=new g(a,{color:this.colors[a]});console.log(this.letters);this.scrollme=null;this.previous_target=0;this.rendered=[];this.previous_zoom=0;this.render=function(c){if(this.data){c=c||{};var a=c.zoom||this.zoom,b=c.target||1,u=d(this.dom_element).parent().width(),f=1,e=0;if(b!==this.previous_target){this.previous_target=b;c.start&&(this.start=c.start);c.end&&(this.end=c.end);0.1>=a?a=0.1:1<=a&&(a=1);this.zoom=a;c=this.end||
this.data.height_arr.length;var h=this.start||1;c=c>this.data.height_arr.length?this.data.height_arr.length:c;c=c<h?h:c;h=h>c?c:h;h=1<h?h:1;this.y=this.height-20;this.max_width=this.column_width*(c-h+1);u>this.max_width&&(a=1,this.zoom_enabled=!1);this.zoom=a;this.zoomed_column=this.column_width*a;this.total_width=this.zoomed_column*(c-h+1);if(1>a)for(;this.total_width<u&&!(this.zoom+=0.1,this.zoomed_column=this.column_width*this.zoom,this.total_width=this.zoomed_column*(c-h+1),this.zoom_enabled=
!1,1<=a););b>this.total_width&&(b=this.total_width);d(this.dom_element).attr({width:this.total_width+"px"}).css({width:this.total_width+"px"});u=Math.ceil(this.total_width/this.canvas_width);this.columns_per_canvas=Math.ceil(this.canvas_width/this.zoomed_column);this.previous_zoom!==this.zoom&&(d(this.dom_element).find("canvas").remove(),this.previous_zoom=this.zoom,this.rendered=[]);this.canvases=[];this.contexts=[];for(e=0;e<u;e++){var g=this.columns_per_canvas*e+h,n=g+this.columns_per_canvas-1;
n>c&&(n=c);var q=(n-g+1)*this.zoomed_column;q>f&&(f=q);var s=f*e,w=s+q;if(b<w+w/2&&b>s-s/2&&1!==this.rendered[e]){var s=this.canvases,B=e,v=this.dom_element,t=this.height,l=q,C=e,D=f,z=d(v).find("#canv_"+C);z.length||(d(v).append('<canvas class="canvas_logo" id="canv_'+C+'"  height="'+t+'" width="'+l+'" style="left:'+D*C+'px"></canvas>'),z=d(v).find("#canv_"+C));d(z).attr("width",l).attr("height",t);k()||(z[0]=G_vmlCanvasManager.initElement(z[0]));s[B]=z[0];this.contexts[e]=this.canvases[e].getContext("2d");
this.contexts[e].setTransform(1,0,0,1,0,0);this.contexts[e].clearRect(0,0,q,this.height);this.contexts[e].fillStyle="#ffffff";this.contexts[e].fillRect(0,0,w,this.height);12<this.zoomed_column?(q=parseInt(10*a,10),q=10<q?10:q,this.debug&&this.render_with_rects(g,n,e,1),this.render_with_text(g,n,e,q)):this.render_with_rects(g,n,e);this.rendered[e]=1}}!this.scrollme&&k()&&(this.scrollme=new EasyScroller(d(this.dom_element)[0],{scrollingX:1,scrollingY:0,eventTarget:this.called_on}));1!==b&&k()&&this.scrollme.reflow()}}};
this.render_x_axis_label=function(){d(this.dom_element).parent().before('<div class="logo_xaxis" class="centered" style="margin-left:40px"><p class="xaxis_text" style="width:10em;margin:1em auto">Model Position</p></div>')};this.render_y_axis_label=function(){d(this.dom_element).parent().before('<canvas class="logo_yaxis" height="300" width="40"></canvas>');var c=d(this.called_on).find(".logo_yaxis");Math.abs(this.data.max_height);isNaN(this.data.min_height_obs)||parseInt(this.data.min_height_obs,
10);k()||(c[0]=G_vmlCanvasManager.initElement(c[0]));c=c[0].getContext("2d");c.beginPath();c.moveTo(40,1);c.lineTo(30,1);c.moveTo(40,256);c.lineTo(30,256);c.moveTo(40,128);c.lineTo(30,128);c.lineWidth=1;c.strokeStyle="#666666";c.stroke();c.fillStyle="#666666";c.textAlign="right";c.font="bold 10px Arial";c.textBaseline="top";c.fillText(this.data.max_height.toFixed(1),28,0);c.textBaseline="middle";c.fillText(parseFloat(this.data.max_height/2).toFixed(1),28,128);c.fillText("0",28,256);c.save();c.translate(5,
this.height/2);c.rotate(-Math.PI/2);c.textAlign="center";c.font="normal 12px Arial";c.fillText("Information Content",1,0);c.restore()};this.render_x_axis_label();this.render_y_axis_label();this.render_with_text=function(c,a,d,g){var p=0,r=c,h=0,h=Math.abs(this.data.max_height),m=isNaN(this.data.min_height_obs)?0:parseInt(this.data.min_height_obs,10),h=h+Math.abs(m),h=Math.round(100*Math.abs(this.data.max_height)/h);Math.round(256*h/100);a+3<=this.end&&(a+=3);for(h=c;h<=a;h++){if(this.data.mmline&&
1===this.data.mmline[h-1])this.contexts[d].fillStyle="#cccccc",this.contexts[d].fillRect(p,10,this.zoomed_column,this.height-40);else if(c=this.data.height_arr[h-1])for(var m=0,n=c.length,q=0,q=0;q<n;q++){var s=c[q],w=s.split(":",2),l=p+this.zoomed_column/2,v=null;if(0.01<w[1]){var v=parseFloat(w[1])/this.data.max_height,w=255-m,t=255*v;k()||(w+=t*(v/2));this.letters[s[0]].draw(this.contexts[d],t,this.zoomed_column,l,w);m+=t}}f(this.contexts[d],p,this.height-15,5);f(this.contexts[d],p,this.height-
30,5);f(this.contexts[d],p,this.height-45,5);0.7>this.zoom?0===h%5&&this.draw_column_divider({context_num:d,x:p,fontsize:10,column_num:r,ralign:!0}):this.draw_column_divider({context_num:d,x:p,fontsize:g,column_num:r});c=this.data.delete_probs[h-1]/100;m=this.height-35;n="#ffffff";q="#555555";0.25<c?(n="#2171b5",q="#ffffff"):0.15<c?n="#6baed6":0.05<c&&(n="#bdd7e7");e(this.contexts[d],p,m,c,g,this.zoomed_column,n,q);c=this.contexts[d];m=p;n=this.height;q=this.zoomed_column;s=this.data.insert_probs[h-
1]/100;l=n-20;v="#ffffff";w="#555555";0.1<s?(v="#d7301f",w="#ffffff"):0.05<s?v="#fc8d59":0.03<s&&(v="#fdcc8a");e(c,m,l,s,g,q,v,w);0.1<s&&f(c,m+q,n-30,-30-n,v);c=this.data.insert_lengths[h-1];m="#ffffff";n="#555555";9<c?(m="#d7301f",n="#ffffff"):7<c?m="#fc8d59":4<c&&(m="#fdcc8a");e(this.contexts[d],p,this.height-5,c,g,this.zoomed_column,m,n);p+=this.zoomed_column;r++}b(this.contexts[d],this.height-15,this.total_width);b(this.contexts[d],this.height-30,this.total_width);b(this.contexts[d],this.height-
45,this.total_width);b(this.contexts[d],0,this.total_width)};this.draw_column_divider=function(c){var a=c.ralign?c.x+this.zoomed_column:c.x,b=c.ralign?c.x+2:c.x;f(this.contexts[c.context_num],a,this.height-30,-30-this.height,"#dddddd");f(this.contexts[c.context_num],a,0,5);var a=this.contexts[c.context_num],u=this.zoomed_column,e=c.column_num,d=c.ralign;a.font=c.fontsize+"px Arial";a.textAlign=d?"right":"center";a.fillStyle="#666666";a.fillText(e,b+u/2,10)};this.render_with_rects=function(c,a,e,d){var g=
0,r=c,h=0,h=Math.abs(this.data.max_height),m=Math.abs(this.data.min_height_obs),h=h+m,h=Math.round(100*Math.abs(this.data.max_height)/h);Math.round(256*h/100);m=10;for(h=c;h<=a;h++){if(this.data.mmline&&1===this.data.mmline[h-1])this.contexts[e].fillStyle="#cccccc",this.contexts[e].fillRect(g,10,this.zoomed_column,this.height-40);else{c=this.data.height_arr[h-1];for(var n=0,q=c.length,s=0,s=0;s<q;s++){var k=c[s].split(":",2);if(0.01<k[1]){var l=parseFloat(k[1])/this.data.max_height,v=g,l=256*l,t=
256-n-l;d?(this.contexts[e].strokeStyle=this.colors[k[0]],this.contexts[e].strokeRect(v,t,this.zoomed_column,l)):(this.contexts[e].fillStyle=this.colors[k[0]],this.contexts[e].fillRect(v,t,this.zoomed_column,l));n+=l}}}0.2>this.zoom?m=20:0.3>this.zoom&&(m=10);0===h%m&&(f(this.contexts[e],g+this.zoomed_column,this.height-30,parseFloat(this.height),"#dddddd"),f(this.contexts[e],g+this.zoomed_column,0,5),c=this.contexts[e],n=g-2,q=this.zoomed_column,s=r,c.font="10px Arial",c.textAlign="right",c.fillStyle=
"#666666",c.fillText(s,n+q/2,10));c=this.contexts[e];n=g;q=this.height-42;s=this.zoomed_column;k=this.data.insert_probs[h-1]/100;v=this.data.insert_lengths[h-1];l=this.data.delete_probs[h-1]/100;t="#ffffff";0.1<k?t="#d7301f":0.05<k?t="#fc8d59":0.03<k&&(t="#fdcc8a");c.fillStyle=t;c.fillRect(n,q+12,s,10);t="#ffffff";9<v?t="#d7301f":7<v?t="#fc8d59":4<v&&(t="#fdcc8a");c.fillStyle=t;c.fillRect(n,q+24,s,10);t="#ffffff";0.25<l?t="#2171b5":0.15<l?t="#6baed6":0.05<l&&(t="#bdd7e7");c.fillStyle=t;c.fillRect(n,
q,s,10);b(this.contexts[e],this.height-45,this.total_width);b(this.contexts[e],0,this.total_width);g+=this.zoomed_column;r++}};this.toggle_scale=function(){var c=this.current_column();this.data.max_height=this.data.max_height===this.data.max_height_obs?this.data.max_height_theory:this.data.max_height_obs;this.rendered=[];d(this.called_on).find(".logo_yaxis").remove();this.render_y_axis_label();this.scrollme.reflow();this.scrollToColumn(c+1);this.scrollToColumn(c)};this.current_column=function(){var c=
this.scrollme.scroller.getValues().left,a=this.column_width*this.zoom,c=c/a,a=d(this.called_on).find(".logo_container").width()/a/2;return Math.ceil(c+a)};this.change_zoom=function(c){var a=0.3;c.target?a=c.target:c.distance&&(a=(parseFloat(this.zoom)-parseFloat(c.distance)).toFixed(1),"+"===c.direction&&(a=(parseFloat(this.zoom)+parseFloat(c.distance)).toFixed(1)));1<a?a=1:0.1>a&&(a=0.1);d(this.called_on).find(".logo_graphic").width()*a/this.zoom>d(this.called_on).find(".logo_container").width()&&
(c.column?(this.zoom=a,this.render({zoom:this.zoom}),this.scrollme.reflow(),a=this.coordinatesFromColumn(c.column),this.scrollme.scroller.scrollTo(a-c.offset)):(c=this.current_column(),this.zoom=a,this.render({zoom:this.zoom}),this.scrollme.reflow(),this.scrollToColumn(c)));return this.zoom};this.columnFromCoordinates=function(a){return Math.ceil(a/(this.column_width*this.zoom))};this.coordinatesFromColumn=function(a){return(a-1)*this.column_width*this.zoom+this.column_width*this.zoom/2};this.scrollToColumn=
function(a,b){var e=d(this.called_on).find(".logo_container").width()/2,u=this.coordinatesFromColumn(a);this.scrollme.scroller.scrollTo(u-e,0,b)}}var b=null;d.fn.hmm_logo=function(a){var b=null,f=d('<div class="logo_graphic">');if(k()){a=a||{};d(this).append(d('<div class="logo_container">').append(f).append('<div class="logo_divider">'));a.data=d(this).data("logo");if(null===a.data)return;a.dom_element=f;a.called_on=this;var e=d('<form class="logo_form"><fieldset><label for="position">Column number</label><input type="text" name="position" class="logo_position"></input><button class="button logo_change">Go</button></fieldset></form>'),
b=new l(a);b.render(a);b.scale_height_enabled&&b.data.max_height_obs<b.data.max_height_theory&&e.append('<button class="logo_scale button">Toggle Scale</button><br/>');b.zoom_enabled&&e.append('<button class="logo_zoomout button">-</button><button class="logo_zoomin button">+</button>');d(this).append(e);d(this).find(".logo_reset").bind("click",function(a){a.preventDefault();a=b;a.change_zoom({target:a.default_zoom})});d(this).find(".logo_change").bind("click",function(a){a.preventDefault()});d(this).find(".logo_zoomin").bind("click",
...
a.pageX-d(this).parent().offset().left;f=e.columnFromCoordinates(f);1>e.zoom?e.change_zoom({target:1,offset:a,column:f}):e.change_zoom({target:0.3,offset:a,column:f})});a.column_info&&f.bind("click",function(c){var e=b,f=d('<table class="logo_col_info"></table>'),g="<tr>",k="",l=d(this).offset();c=parseInt(c.pageX-l.left,10);d(this).parent().offset();var l=e.columnFromCoordinates(c),e=[],h=0,h=c=0,m="Probability";b.data.height_calc&&"score"===b.data.height_calc?(m="Score",e=b.data.height_arr[l-1].slice(0).reverse()):
e=b.data.probs_arr[l-1].slice(0).reverse();h=Math.ceil(e.length/5);for(c=0;c<h;c++)g+="<th>Residue</th><th>"+m+"</th>";f.append(d(g+"</tr>"));for(c=0;5>c;c++){k+="<tr>";for(h=c;e[h];)g=e[h].split(":",2),k+="<td>"+g[0]+"</td><td>"+g[1]+"</td>",h+=5;k+="</tr>"}f.append(d(k));d(a.column_info).empty().append(f)});d(document).bind(this.attr("id")+".scrolledTo",function(a,e,f,d){b.render({target:e})});d(document).keydown(function(a){a.ctrlKey||(61!==a.which&&107!==a.which||b.change_zoom({distance:0.1,direction:"+"}),
109!==a.which&&0!==a.which||b.change_zoom({distance:0.1,direction:"-"}))})}else d("#logo").replaceWith(d("#no_canvas").html());return b}})(jQuery);/*
...
function(b,c){var d=u++;a[d]=b;null===f&&(f=setTimeout(function(){var b=k(),c=a,e=g(c);a={};f=null;for(var d=0,u=e.length;d<u;d++)c[e[d]](b)},1E3/60));return d};d.cancelRequestAnimationFrame=function(b){delete a[b];l(a)&&(clearTimeout(f),f=null)}}}})(this);
